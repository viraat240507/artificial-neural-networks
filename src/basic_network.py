...
class NeuralNetworks:
    ...
    def backpropagation(self,epochs=100,learning_rate=0.01):
        ...
        for e in range(epochs):
            ...
            y_actual=np.array(self.y_actual).reshape(y_pred.shape)
            
            dA=self.loss_fn.gradient(y_actual,y_pred)
            ...
            for i in range(len(self.layers)-1,-1,-1):
                ...
                curr_layer.W-=learning_rate*dW
                curr_layer.b-=learning_rate*db
            y_pred=self.forward_prop()
